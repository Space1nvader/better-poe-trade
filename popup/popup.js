...
const setActivePoeSection = (section) => {
    if(chrome.storage.local){
        chrome.storage.local.set({['active-poe']: section});
    }
}

const getFavorites = async (key = FAVORITES_STORAGE_KEY) => {
    if(chrome?.storage?.local){
        const favoritesExtensionStorage = await chrome.storage.local.get(key)
        const favorites = favoritesExtensionStorage[key] || []
        ...
        favorites.forEach(favorite => {
            ...
            const deleteButton = createDeleteButton(favorites, favorite.name, favoriteItem, key);
...
const buttons = document.querySelectorAll('.poe-switch-button');

function poeSwitchButtonHandler(event){
    const targetButton = event.target.getAttribute('data-active');

    if(!targetButton) return;

    setActivePoeSection(targetButton);

    document.querySelectorAll('.poe-switch-button').forEach(button => {
        if(button.getAttribute('data-active') === targetButton){
            button.classList.add('active');
        } else {
            button.classList.remove('active');
        }
      });
    getFavorites(targetButton);
}

const setButtonActive = async () => {
    const activePoeSection = await chrome.storage.local.get('active-poe')

    const targetSection = activePoeSection['active-poe'] || 'favorites'
    console.log('activePoeSection', targetSection);
    const targetButton = document.querySelector(`.poe-switch-button[data-active="${targetSection}"]`)
    
    console.log(targetButton);
    if(targetButton) {
        targetButton.classList.add('active');

        getFavorites(targetSection);
    }
...
window.addEventListener('click', poeSwitchButtonHandler);